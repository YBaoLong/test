   ...
   <!-- end: Header -->
	...
	<div class="container-fluid content">
		...
		<div class="main ">
            ...
            <div class="row">
				<div class="col-lg-12">
					<h3 class="page-header"><i class="fa fa-pencil"></i>入馆人员信息</h3>
				...
				<div class="panel panel-default">
					<div class="panel-body">
						<table class="table table-striped table-responsive">
							<thead>
								<tr>
									...
									<!--  
									<th class="center">入馆限制1</th>
									<th class="center">入馆限制2</th>
									<th class="center">入馆限制3</th>
									-->
							...
							<tbody>
				               ...
				               <c:forEach items="${pageBean.list}" var="list" varStatus="status">
								<tr>
								    ...
								    <!--  
								    <c:choose>
								        <c:when test="{list.limit1=='正常'}">
								        <td class="center">${list.limit1}</td>
								        </c:when>
								        <c:otherwise>
								         <td class="center" bgcolor="red">${list.limit1}</td>
								        </c:otherwise>
								    </c:choose>
									  <c:choose>
								        <c:when test="{list.limit2=='正常'}">
								        <td class="center">${list.limit2}</td>
								        </c:when>
								        <c:otherwise>
								         <td class="center" bgcolor="red">${list.limit2}</td>
								        </c:otherwise>
								    </c:choose>
								      <c:choose>
								        <c:when test="{list.limit3=='正常'}">
								        <td class="center">${list.limit3}</td>
								        </c:when>
								        <c:otherwise>
								         <td class="center" bgcolor="red">${list.limit3}</td>
								        </c:otherwise>
								    </c:choose>
						...
						<div class="col-md-12 text-right">
						   <common:page url="user/inform.action" />
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal" id="edit_user_form">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="cash" placeholder="是否交押金" name="cash">
							</div>
						</div>
				...
				<!--  
					...
					<button  class="btn btn-primary" type="submit" onclick="updateUser()">保存</button>
			     -->
			</div><!-- /.modal-content -->
		</div><!-- /.modal-dialog -->
	</div><!-- /.modal -->
